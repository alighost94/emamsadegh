...
// اضافه کردن این متدها به کلاس AdminController
...
public function searchCourses() {
    ...
    $courses = $this->courseModel->searchCourses($filters);
    header('Content-Type: application/json');
    echo json_encode($courses);
    exit;
}

// متد تخصیص گروهی دروس
public function assignMultipleCourses() {
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        $teacher_id = $_POST['teacher_id'] ?? null;
        $courses_data = json_decode($_POST['courses_data'], true) ?? [];
        
        if (!$teacher_id || empty($courses_data)) {
            $_SESSION['error'] = 'لطفاً معلم و دروس را انتخاب کنید';
            $this->redirect('admin/assign_course');
            return;
        }
        
        $assigned_count = $this->teacherCourseModel->assignMultipleCourses($teacher_id, $courses_data);
        
        if ($assigned_count !== false) {
            // ثبت لاگ فعالیت
            $this->logActivity(
                'multiple_course_assignment',
                "تخصیص $assigned_count درس به معلم",
                $teacher_id,
                'teacher_courses'
            );
            
            $_SESSION['success'] = "تعداد $assigned_count درس با موفقیت به معلم تخصیص داده شد";
        } else {
            $_SESSION['error'] = 'خطا در تخصیص دروس';
        }
        
        $this->redirect('admin/assign_course');
    }
}
// اضافه کردن این متد به AdminController
public function getCourseDetails($course_id) {
    $course = $this->courseModel->getCourseWithDetails($course_id);
    header('Content-Type: application/json');
